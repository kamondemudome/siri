...
import joblib
from utils import predict_diabetes, categorize_risk, get_health_avatar, get_health_tips, generate_pdf_report, FEATURE_FULL_NAMES, FEATURE_DESCRIPTIONS, FEATURE_TOOLTIPS, AGE_LABELS, INCOME_LABELS, FEATURES, save_to_csv, apply_settings, load_settings
...
# Apply settings and custom CSS at the start of the app
...
def load_model():
    ...
    if os.path.exists(model_path):
        ...
        st.error("Model file 'new_diabetes_rf_model.pkl' not found. Please run the training script first.")

# Diabetes Detection Tool Page Content
def main():
    ...
    if st.session_state.get("prediction_model") is None:
        ...
        st.markdown("""
        This tool predicts your risk of diabetes based on 10 common health and lifestyle indicators.
        ...
        st.session_state.setdefault("settings", {"theme": "Light"})  # Default to Light theme

        # Form for user input with reduced 10 indicators
        with st.form("input_form"):
            ...
            with col1:
                ...
                # Calculate BMI dynamically and validate
                if height > 0:
                    bmi = round(weight / (height ** 2), 1)
                    if np.isfinite(bmi):
                        user_data['BMI'] = bmi
                    else:
                        user_data['BMI'] = 25.0  # Fallback for invalid BMI
                    st.write(f"**Calculated BMI:** {user_data['BMI']}")
                else:
                    user_data['BMI'] = 25.0
                    st.write("**Calculated BMI:** 25.0 (Please enter a valid height)")
                user_data['GenHlth'] = st.radio(FEATURE_DESCRIPTIONS['GenHlth'], options=[5, 4, 3, 2, 1],
        ...
        if submitted:
            try:
                ...
                # Validate user_data before prediction
                expected_features = ['HighBP', 'HighChol', 'BMI', 'GenHlth', 'Smoker', 
