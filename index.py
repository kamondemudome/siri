...
# Load the prediction model with enhanced debugging in the background
...
def main():
    ...
    @keyframes pulse {
        ...
        50% {
            ...
            box-shadow: 0 0 15px 5px rgba(255, 87, 34, 0.7); /* Orange glow */
            opacity: 1;
        }
        100% {
            transform: scale(1);
            box-shadow: 0 0 0 0 rgba(26, 115, 232, 0.7); /* Blue glow */
            opacity: 0.9;
        }
    }
    .metric {
    ...
    <div style='margin-top: -20px; margin-bottom: 20px;'>
        <a href='#' style='text-decoration: none;'>
            <button style='font-size: 18px; padding: 10px 30px;'>
                🚀 Start Full Risk Assessment
            </button>
        </a>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with col1:
            ...
            if model is None:
                st.error("The prediction model is not available. Please ensure 'new_diabetes_rf_model.pkl' is in D:/Myproject/ and restart the app. You can still explore other sections like Home, Awareness, and Preventive Measures.")
            else:
                ...
                if submit:
                    level, prob = quick_predict_diabetes(bmi, activity == "Yes", fruit == "Yes", (age - 18)//7 + 1, model)
        ...
        with col2:
            ...
            st.image("D:/Myproject/pages/healthy_lifestyle.jpg", width=300)  # Removed caption
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button("Clear Chat"):
        st.session_state["ai_messages"] = []
        st.rerun()  # Refresh to reflect cleared chat
    st.markdown('</div>', unsafe_allow_html=True)
